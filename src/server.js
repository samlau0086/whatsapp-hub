...
app.get("/agent/baileys-client.js", (req, res) => {
  res.type("text/javascript").sendFile(path.join(__dirname, "..", "agents", "wwebjs-client", "index.js"));
});
...
app.patch("/admin/api/client-configs/:id", requireWebSession, requirePermission("clients:delete"), (req, res) => {
  ...
  const clientConfig = updateClientConfig(req.params.id, {
    ...
    authDataPath: String(payload.authDataPath || current.auth_data_path || `./.baileys_auth_${current.client_id}`).trim(),
    cachePath: String(payload.cachePath || current.cache_path || `./.baileys_store_${current.client_id}`).trim(),
...
app.post("/admin/api/client-configs", requireWebSession, requirePermission("clients:delete"), (req, res) => {
  ...
  const clientConfig = createClientConfig({
    ...
    authDataPath: String(payload.authDataPath || `./.baileys_auth_${clientId}`).trim(),
    cachePath: String(payload.cachePath || `./.baileys_store_${clientId}`).trim(),
    proxyUrl: String(payload.proxyUrl || "").trim(),
    proxyUsername: String(payload.proxyUsername || "").trim(),
    proxyPassword: String(payload.proxyPassword || ""),
    headless: payload.headless !== false,
...
function agentPackageJson() {
  return {
    ...
    version: "0.2.0-baileys.6",
    private: true,
    type: "module",
    scripts: {
      start: "node baileys-client.js"
    },
    dependencies: {
...
function buildClientDeployment(clientConfig, token, fallbackHubUrl = config.publicBaseUrl) {
  ...
  const resolved = {
    ...
    authDataPath: clientConfig.auth_data_path || `./.baileys_auth_${clientConfig.client_id || "client-main"}`,
    cachePath: clientConfig.cache_path || `./.baileys_store_${clientConfig.client_id || "client-main"}`,
  ...
  const env = [
    ...
    ["CLIENT_TOKEN", token],
    ["BAILEYS_AUTH_DATA_PATH", resolved.authDataPath],
    ["BAILEYS_STORE_PATH", resolved.cachePath],
  ...
  return {
    config: resolved,
    env,
    agentUrl: new URL("baileys-client.js", agentBaseUrl).toString(),
...
function buildLinuxAgentScript({ clientId, agentBaseUrl, env }) {
  return [
    ...
    "    return",
    "  fi",
    "  echo \"Git is required by some Baileys dependencies. Installing when possible...\"",
    "  if [ \"$(uname -s)\" = \"Darwin\" ]; then",
    "    if command -v brew >/dev/null 2>&1; then",
    "      brew install git",
    "    else",
    "      echo \"Git is not installed. Install Git or Xcode Command Line Tools, then rerun this script.\" >&2",
    "      exit 1",
    "    fi",
    "  elif command -v apt-get >/dev/null 2>&1; then",
    "    sudo apt-get update",
    ...
    "  else",
    "    echo \"No supported package manager found. Install Git manually, then rerun this script.\" >&2",
    "    exit 1",
    "  fi",
    "  command -v git >/dev/null 2>&1 || { echo \"Git installation failed or PATH was not refreshed.\" >&2; exit 1; }",
    "}",
    "",
    "download_file() {",
    "  url=\"$1\"",
    ...
    "    return",
    "  fi",
    "  echo \"npm install failed. Cleaning partial install, then retrying once...\" >&2",
    ...
    `download_file ${shellSingleQuote(new URL("baileys-client.js", agentBaseUrl).toString())} baileys-client.js`,
    "cat > .env <<'EOF'",
    env,
    "EOF",
    "install_dependencies",
...
function buildWindowsAgentScript({ clientId, agentBaseUrl, env }) {
  return [
    ...
    "if errorlevel 1 (",
    "  echo.",
    "  echo Git is required by some Baileys dependencies, but git.exe was not found.",
    "  echo Please install Git for Windows from https://git-scm.com/download/win and run this BAT again.",
    "  pause",
    "  exit /b 1",
    ")",
    "",
    ...
    "if errorlevel 1 (",
    "  echo.",
    "  echo npm install failed. Cleaning partial install, then retrying once...",
    ...
    "set \"PATH=C:\\Program Files\\Git\\cmd;C:\\Program Files (x86)\\Git\\cmd;%MACHINE_PATH%;%USER_PATH%;%PATH%\"",
    "exit /b 0",
    "",
    ":DownloadFile",
    "set \"DOWNLOAD_URL=%~1\"",
    ...
    "  echo     echo Git is missing. Run the install BAT again, or install Git for Windows from https://git-scm.com/download/win.",
    "  echo     pause",
    "  echo     exit /b 1",
    "  echo   ^)",
    ...
    "echo If the log contains \"spawn git\" or \"path git\", install Git for Windows and rerun this BAT.",
    "pause",
    "exit /b 1",
    "",
    ":Failed",
